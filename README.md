...
Component Communication
App invokes data hooks to load capital gains and holdings.
HarvestingProvider is initialized with baseline gains.
HoldingsTable triggers toggleHolding / toggleAll from context.
Context applies applyHoldingToGains and recalculates derived gains.
CapitalGainsCard re-renders with updated postHarvestGains.
System Architecture Diagram
...
Build:
npm run build
Deploy the generated build/ folder to any static host (Nginx, S3+CloudFront, Netlify, Vercel static output, etc.).
...
Include screenshots for UI-impacting changes (desktop + mobile + dark/light modes).
